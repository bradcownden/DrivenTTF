...
\begin{align}
\label{phi1 eqn}
\p^2_t \phi_1 + \hat L \phi_1 = \mc F(t) \quad \text{where} \quad \hat{L} \equiv \frac{1}{\mu} (\mu' \p_x + \mu \p^2_x)  - \frac{m^2}{\cos^2(x)} \, ,
\end{align}
...
\end{align}
Requiring regularity at the origin -- and without imposing any restrictions on the frequencies $\omega_I$ -- we
find that \cite{Nastase}
\begin{align}
	\label{general basis}
...
the boundary, denoted $\Phi^\pm_I (x)$, that combine to give the solution at the origin through $E_I (x) = C_I^+ \Phi^+_I(x) + C^{-}_I \Phi^{-}_I (x)$.
By examining each function's scaling when $x \to \pi/2$,  we see that $\Phi_I^+$ is normalizable and goes as $(\cos x )^{\Delta^+}$\!\! while $\Phi_I^-$ is non-normalizable and goes as $( \cos x )^{\Delta^-}$. 
It is these non-normalizable modes that couple to the inhomogeneous part of \eqref{phi1 eqn}. Thus, the general solution for the
scalar field in the bulk can be written as a sum of normalizable \emph{and} non-normalizable components.
