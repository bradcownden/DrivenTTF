...
\abstract{We extend the study of the non-linear perturbative theory of energy cascades in AdS$_{d+1}$ to include solutions of driven systems, i.e.~those with time-dependent sources on the AdS boundary. This necessitates the activation of non-normalizable modes in the linear solution for the massive bulk scalar field, which couple to the metric and normalizable scalar modes. We determine analytic expressions for secular terms in the renormalization flow equations for any mass, and for various driving functions. Finally, we numerically evaluate these sources for $d=4$ and discuss what role these driven solutions play in the perturbative stability of AdS.}
...
\begin{align}
\label{phi1 gen}
\phi_1(t,x) &= \sum_I a_I \cos \left( \omega_I t + b_I \right) \nonumber \\
&= \sum_j a_j (t) \cos \left( \oi t + b_i(t) \right) e_j(x) + \sum_\alpha \bar A_\alpha (t) \cos \left( \omega_\alpha t + \bar{B}_\alpha(t) \right) E_\alpha(x) \, .
\end{align}
The values of $\bar A_\alpha(t)$ and $\bar{B}_\alpha(t)$ are set by the choice of boundary conditions. For example, if the driving term on the boundary is a single, periodic function, then
\begin{align}
\label{BC}
...
\end{align}
Following the steps outlined in Appendix~\ref{app: source term derivation}, we project \eqref{3rd order} onto the basis of normalizable modes and, employing a ubiquitous time-dependent solution ${c_I(t) = a_I \cos (\omega_I t + b_I) = a_I \cos\theta_I}$ with $I \in \{i, \alpha \}$, we find that the source term for the $\ell^{\rm th}$ normalizable mode is
\begin{align}
...
\label{sec: norm res}
Consider the case where each of the basis functions are given by normalizable solutions. Resonances occur for the following combination of normalizable frequencies: 
\begin{align}
\label{gen res}
