...
With this in mind, we examine the effects that a time-dependent source on the conformal boundary has on the non-linear perturbative theory. The introduction of a driving term on the boundary means that we must include a second class of fundamental modes with arbitrary frequencies \cite{hep-th/9805171}. Since these solutions will have non-finite inner products over the bulk space, they are known as non-normalizable. Non-normalizable modes are identified through the AdS/CFT correspondence with sources coupling to boundary operators \cite{hep-th/9802150,  hep-th/9802109}.
...
Let us first consider a spherically symmetric, minimally coupled, massive scalar field on an asymptotically AdS$_{d+1}$ spacetime in global coordinates whose metric is given by
\begin{align}
\label{AdS metric}
...
\begin{align}
\label{phi1 eqn}
\p^2_t \phi_1 + \hat L \phi_1 = 0 \quad \text{and}& \quad \phi(t, x \to \pi/2) = \mc F(t) \left( \cos(x) \right)^{\Delta^-}\, \\
...
The non-normalizable modes are the ones that couple to the inhomogeneous boundary condition for the scalar field \cite{Nastase}. 
...
\end{align}
with the Jacobi polynomials $P^{(a,b)}_n (x)$ providing an orthogonal basis that satisfies ${\langle e_i (x), e_j(x) \rangle = \delta_{i j}}$ with respect to the inner product
\begin{align}
	\langle f(x), g(x) \rangle = \int^{\pi/2}_0 dx \, \mu(x) \bar f(x) g(x) \, ,
\end{align}
when
\begin{align}
...
time-dependent part of the boundary Hamiltonian. To illustrate this, we write the field as a linear combination of normalizable and
...
\end{align}
Therefore, the presence of non-normalizable modes corresponds to pumping energy into and out of the boundary theory. In the presence of the 
...
-------- \\
Inclusion of counterterms on the boundary in order to calculate the bulk to boundary correlator \cite{hep-th/9804058} \\
-------- \\
