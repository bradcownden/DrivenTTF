...
\begin{align}
\phi_1(t,x) = \sum_j c_j(t) e_j(x) + \sum_\alpha A_\alpha (t) E_\alpha(x) \, ,
\end{align}
with the $A_\alpha$ set by the choice of boundary conditions. The normalizable modes have eigenfunctions given by
\begin{align}
...
\begin{center}
{\bf PLOT OF $\Gamma$ AND $\Omega$ FOR MASSIVE SCALARS?}
\end{center}
...
\begin{center}
{\bf PLOT OF $S, R, T$ FOR MASSIVE SCALARS?}
\end{center}
...
%
& \quad + \bigg[ \Theta\left( n - \ell - d \right) \overline{R}^{(-+)}_{(n - \ell - d) 1 2 \ell} \cos \left( \theta_{(n - \ell - d)} + 2nt \right) \bigg]_{m^2 = 0} + \overline{T}_{12\ell} \cos \left( \theta_\ell \right) \, ,
\end{align}
...
\begin{align}
S_\ell &= a_\ell A^2_{\ob} \left( \overline{R}^{(1)}_{\ell \ob} \cos \left( \theta_\ell - 2\ob t \right) + \overline{R}^{(2)}_{\ell\ob} \cos \left( \theta_\ell + 2\ob t \right) + \overline{R}^{(3)}_{\ell \ob} \cos \left( 2\ob t - \theta_\ell \right) \right)\end{align}
