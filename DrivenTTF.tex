...
In addition to the case of arbitrary values of $\ob$, which is discussed in \S\!~\ref{ssec: equalNN}, the following special values of $\ob$ contribute to respective $\overline T$-type coefficients to the source term
\begin{align}
\label{gen NN res}
...
\end{align}
with $\omega_i \neq \omega_\ell$ in each case. The sum of these resonances contribute to the source term $S_\ell$ via
\begin{align}
...
& \quad + \bar A^2_{\ob} \, \overline{T}^{(3)}_{(\ob - \ell - \Delta^+)} \, a_{(\ob - \ell- \Delta^+)} \cos \left( 2\ob t - \theta_{(\ob - \ell - \Delta^+)} \right)  + 3 \bar A^2_{\ob} \, \overline{V}_\ell \, a_\ell \cos \left( \theta_\ell \right) 
\end{align}
under their respective conditions on the value of $\ob$. Evaluating \eqref{general source} in each case, we see that
\begin{align}
...
\end{align}
with $\overline{V}_\ell$ common to all three channels:
\begin{align}
\overline{V}_\ell &= \frac{1}{2} \ol^2 X_{\ob\ob\ell\ell} - \frac{1}{2}\ob^2 X_{\ell\ell\ob\ob} + \frac{m^2}{2} V_{\ob\ob\ell\ell} + \frac{1}{2} Y_{\ob\ob\ell\ell} + \frac{1}{2} Y_{\ell\ell\ob\ob} \nonumber \\
%
& + \ob^2\ol^2 P_{\ob\ob\ell} - 3 \ob^2\ol^2 P_{\ell\ell\ob} + \ob^2 B_{\ell\ell\ob} + \ol^2 B_{\ob\ob\ell} \, .
\end{align}
