...
\begin{align}
S_\ell = \sum_{\substack{i = 0 \\ \oi \neq \ob \\ \ol \geq 2\ob}}^\infty \overline{\Omega}_{i{\ob}{\ob}\ell} \, a_i a_{\ob} a_{\ob} \cos \left(\thi + \theta_{\ob} + \theta_{\ob} \right) + \ldots
\end{align}
where
\begin{align}
...
Next, consider the resonance condition $\oi - \oj - \ok = \ol$. When we choose either $\oj$ or $\ok$ to be normalizable, we find, for example, that $\oi - \oj - \ok = \ob - \ob - \ok =  \ol$. However, we have chosen all frequencies to be strictly positive, and therefore there are no possible solutions. Next, we choose $\oi$ to be the frequency of a normalizable mode, with $\oj = \ok = \ob$ being non-normalizable. The resonant contribution takes the form
\begin{align}
S_\ell = \sum_{\substack{i = 0 \\ \oi, \ol \, \neq \, \ob}}^\infty \overline{\Gamma}^{(i)}_{i {\ob}{\ob} \ell} \, a_i a_{\ob} a_{\ob} \cos \left(\thi - \theta_{\ob} - \theta_{\ob} \right) + \ldots
\end{align}
where
\begin{align}
