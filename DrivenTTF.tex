...
Using these two expressions, we evaluate $\overline{T}_\ell$ for $\ell < 10$ and a variety of $\ob$ values for both a massless scalar (figure~\ref{fig:equal_frequency_m0}) and a tachyon (figure~\ref{fig:equal_frequency_m-05}).

\begin{figure}
\centering
\includegraphics[width=\textwidth]{./figures/NN_equalfreq_sourceterms_m0}
\caption{{\it Left:} Evaluating \eqref{S:2NN} (rescaled by the amplitudes) when $m^2 = 0$ for various choices of $\ob$. {\it Right, top}: Zoom near $S=0$ (dotted line) showing the transition to negative values for the same choices of $\ob$. {\it Right, bottom}: The absolute value of the sum of $S_\ell$ for $\ell < 10$ corresponding to the choices of $\ob$.}
\label{fig:equal_frequency_m0}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{./figures/NN_equalfreq_sourceterms_m-05}
\caption{{\it Left:} Evaluating \eqref{S:2NN} for the same choices of $\ob$ as in figure~\ref{fig:equal_frequency_m0} but for a tachyon with $m^2 = -0.25$.}
\label{fig:equal_frequency_m-05}
\end{figure}
...
\begin{align}
S_\ell &=\Theta \left( \ell - n \right)  \overline{R}^{(1)}_{(\ell - n) 1 2 \ell} \cos \left( \theta_{(\ell - n)} + 2nt \right) + \overline{R}^{(2)}_{(\ell + n) 1 2 \ell } \cos\left( \theta_{(\ell + n)} - 2nt \right) \nonumber \\
& \quad + \delta(m^2) \Theta\left( n - \ell - d \right) \overline{R}^{(3)}_{(n - \ell - d) 1 2 \ell} \cos \left( \theta_{(n - \ell - d)} + 2nt \right) \, ,
\end{align}
...
\begin{align}
\label{R1}
\overline{R}^{(1)}_{i 1 2 \ell} &= - \frac{1}{6} \sum_{\oone + \otwo = 2n} \Big\{ \frac{\otwo}{\ol - \otwo} Z^{-}_{i12\ell} + \frac{\oone}{\ol - \oone} Z^{-}_{i21\ell} + \frac{\ol - 2n}{2n} Z^-_{12i\ell} + \oone \otwo X_{i12\ell}  \nonumber \\
%
...
%
& \quad + \frac{(\ol - 2n)}{\ol - \otwo} \left( H_{2i1\ell} + m^2 V_{i12\ell} - 2\otwo^2 X_{2i1\ell} \right) \Big\} - \frac{1}{6} \sum_{\oone + \otwo = 2n}^{\oi \neq \otwo} \Big\{  \frac{\oone}{\ol - \otwo} \left( H_{i21\ell} \right. \nonumber \\
%
& \quad \left. + \,m^2 V_{21i\ell} - 2\oone^2 X_{21i\ell} \right) + \frac{(\ol - 2n)}{\ol - \oone} \left( H_{2i1\ell} + m^2 V_{i 12 \ell} - 2 \oone^2 X_{2i1\ell} \right) \Big\} \nonumber \\
%
