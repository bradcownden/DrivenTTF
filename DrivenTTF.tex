...
\end{align}
A derivation of the resonant contributions for a massive scalar when the boundary term is zero -- and therefore only normalizable modes are present -- is included as an exercise in Appendix~\ref{sec: norm res}. These results agree numerically with previous work on normalizable modes, namely that two of the three resonance channels vanish naturally, for massless scalars in the interior \cite{1407.6273} and boundary \cite{1412.3249} time gauges, as well as massive scalars in the internior time gauge \cite{1810.04753}. The definitions of the functions $Z$, $H$, $X$, etc.~in~\eqref{general source} differ slightly from these previous works in part because of the gauge choice and in part because of a desire to separate out mass-dependent terms; however, the expressions are made equivalent through applications of integration by parts and setting $m^2 = 0$. To avoid confusion, the definitions of $Z$, $H$, $X$, etc. are given explicitly in appendix~\ref{app: source term derivation}.

It should be emphasized that the contibutions to the source term $S^{(3)}_\ell$ from normalizable modes are \emph{always} present. For driving amplitudes $\mc A \neq 0$, there are additional contributions from non-normalizable modes such that the total $\mc O(\epsilon^3)$ source term is given by the sum of both types of contributions \cite{1810.04753}.
