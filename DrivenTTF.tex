...
resonances contribute to the source term $S_\ell$ via
\begin{align}
...
& \quad + \bar A^2_{\ob} \, \overline{T}^{(3)}_{(\ob - \ell - \Delta^+)} \, a_{(\ob - \ell- \Delta^+)} \cos \left( \theta_{(\ob - \ell - \Delta^+)} - 2 \ob t \right)  + \bar A^2_{\ob} \, \overline{T}_\ell \, a_\ell \cos \left( \theta_\ell \right) 
\end{align}
...
We find that
\begin{align}
\overline{T}^{(2)}_{i} &= - \frac{1}{12} \left(1 - \delta(\ol - \ob) \right) Z^-_{i \ob \ob \ell} \left(\frac{\ob}{\ol - \ob} \right) \nonumber \\
%
