...
Using these two expressions, we evaluate $\overline{T}_\ell$ for $\ell < 10$ and a variety of $\ob$ values for both a massless scalar (figure~\ref{fig:equal_frequency_m0}) and a tachyon (figure~\ref{fig:equal_frequency_m-2_25}).
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{./figures/NN_equalfreq_sourceterms_m-2_25}
\caption{{\it Left:} Evaluating \eqref{S:2NN} for the same choices of $\ob$ as in figure~\ref{fig:equal_frequency_m0} but for a tachyon with $m^2 = -2.25$.}
\label{fig:equal_frequency_m-2_25}
\end{figure}
