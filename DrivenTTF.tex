...
With this in mind, we examine the effects that a time-dependent source on the conformal boundary has on the non-linear perturbative theory. The introduction of a driving term on the boundary means that we must include a second class of fundamental modes with arbitrary frequencies \cite{hep-th/9805171, }. Since these solutions will have non-finite inner products over the bulk space, they are known as non-normalizable. Non-normalizable modes are identified with sources coupling to boundary operators \cite{hep-th/9802150,  hep-th/9802109}.
...
\label{sec: norm res}
In this appendix, we include explicit expressions for contributions to the source term $S_\ell$ from a massive scalar with static boundary conditions ($\mc F(t) = 0$). In this case, only normalizable modes are present. The possible combinations of frequencies that satisfy \eqref{gen res} can be separated into the three distinct cases:
\begin{align}
\oi + \oj + \ok &= \ol \qquad (+++) \\
\oi - \oj - \ok &= \ol \qquad (+--) \\
\oi + \oj - \ok &= \ol \qquad (++-) \, .
